...
class TaucClient:
    ...
    def _oauth_access_token(self) -> str:
        ...
        token_path = "/v1/openapi/token"  # same endpoint for ACS, CLOUD, and OLT
        body_map = {
    ...
    def request(
        ...
        *,
        ...
        # Always sign with HMAC — TP-Link requires X-Authorization even for OAuth2 clients
        # Sign just the path (not including query string) for all oauth2 clients
        signed_path = path
        if self.auth_type != "oauth2" and query_string:
