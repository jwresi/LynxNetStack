...
class BillingControlCenterView(PermissionRequiredMixin, View):
    permission_required = 'netbox_contract.view_contract'
    template_name = 'netbox_contract/billing_control_center.html'
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == 'hardware_checkout':
            self.handle_hardware_checkout(request)
        return redirect('plugins:netbox_contract:billing_control_center')

    def handle_hardware_checkout(self, request):
        if not request.user.has_perm('dcim.change_device'):
            messages.error(request, 'You do not have permission to update hardware assignments.')
            return
        ...
        if not device_id:
            messages.error(request, 'Device is required for checkout.')
            return

        try:
            device = Device.objects.select_related('tenant', 'site').get(pk=device_id)
        ...
        if site_id:
            try:
                ...
                messages.error(request, 'Selected site was not found.')
                return

        previous_tenant = str(device.tenant) if device.tenant else 'Unassigned'
        previous_site = str(device.site) if device.site else 'Unassigned'

        device.tenant = tenant
        if site is not None:
            device.site = site
            # Keep location consistent after a cross-site move.
            device.location = None

        event_note = (
            f"[Billing checkout {timezone.now().strftime('%Y-%m-%d %H:%M:%S')}] "
            f"tenant: {previous_tenant} -> {tenant or 'Unassigned'}; "
            f"site: {previous_site} -> {site or previous_site}"
        )
        if note:
        ...
        update_fields = ['tenant', 'comments']
        if site is not None:
            update_fields.extend(['site', 'location'])
        device.save(update_fields=update_fields)

        messages.success(
            request,
            f"Checkout updated for device {device.name or device.pk}.",
        )

    def get_context_data(self, request):
        invoices_qs = Invoice.objects.select_related().prefetch_related('contracts')
        invoices = (
            invoices_qs
            .order_by('-date', '-period_start')[:12]
        )
        contracts = (
            Contract.objects.select_related('tenant', 'contract_type')
            .annotate(invoice_count=Count('invoices'))
            .order_by('-last_updated')[:12]
        )
        customers = (
            Tenant.objects.annotate(contract_count=Count('contracts'))
            .order_by('name')[:25]
        )
        devices = (
            Device.objects.select_related('tenant', 'site')
            .order_by('name')[:150]
        )
        sites = Site.objects.order_by('name')

        paid_invoices = invoices_qs.filter(status='posted').count()
